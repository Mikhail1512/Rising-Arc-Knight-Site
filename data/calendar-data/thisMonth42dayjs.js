...
export const thisMonth = JSON.parse(localStorage.getItem('thisMonth')) || generateMonthObject(monthArray);

function generateMonthObject(monthArray){
    for (let i = 1; i < 43; i++) {
        ...
        monthsOfTheYear.forEach((monthObject) => {
            if(monthObject.monthName === monthNameJs){
                ...
                monthArray.push({
                    ...
                    hoursObject: structuredClone(dateObject.todaysHours),
                    ...
                    isUpdating: false,
                    dailyEvents: structuredClone(dateObject.eventOfTheDay)
                });
            };
        });
    };
